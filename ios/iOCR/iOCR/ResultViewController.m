...
- (IBAction)testOnline:(id)sender {
    ...
    NSString *encodeding=[[NSString alloc]initWithString:[self encodingStr:result]];

    NSLog(@"%@",encodeding);
    
    [postBody appendData:[encodeding dataUsingEncoding: NSUTF8StringEncoding]];
    [request setHTTPBody:postBody];
    
    NSURLConnection* conn = [[NSURLConnection alloc] initWithRequest: request delegate:self];
    [conn start];
}
-(NSString *)encodingStr: (NSString *)srcStr
{
    NSString *encoding=[[[[srcStr stringByReplacingOccurrencesOfString:@"<" withString:@"%3C"]stringByReplacingOccurrencesOfString:@">" withString:@"%3E"] stringByReplacingOccurrencesOfString:@"/" withString:@"%2F"] stringByReplacingOccurrencesOfString:@"+" withString:@"%2B"];
    return encoding;
}
